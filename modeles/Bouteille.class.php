...
class Bouteille extends Modele
{
    ...
    public function getRechercheBouteilleCellier($recherche, $cellier)
    {
        ...
        $requete = 'SELECT
							...
							INNER JOIN ' . self::CELLIER . ' vc ON c.vino__cellier_id = vc.id
							INNER JOIN ' . self::TYPE . ' t ON t.id = b.fk_type_id
							WHERE LOWER(b.nom) like LOWER("%' . $recherche . '%") AND c.vino__cellier_id=' . $cellier . '
							OR LOWER(b.pays) like LOWER("%' . $recherche . '%") AND c.vino__cellier_id=' . $cellier . '
							OR  c.millesime like ("%' . $recherche . '%") AND c.vino__cellier_id=' . $cellier . '
							OR  LOWER(type) like LOWER("%' . $recherche . '%") AND c.vino__cellier_id=' . $cellier;
    ...
    public function getRechercheBouteilleCatalogue($recherche)
    {
        ...
        $requete = 'SELECT
							...
							FROM ' . self::BOUTEILLE . ' b
							INNER JOIN ' . self::TYPE . ' t ON t.id = b.fk_type_id
							WHERE LOWER(b.nom) like LOWER("%' . $recherche . '%") 
							OR LOWER(b.pays) like LOWER("%' . $recherche . '%")
							OR b.format like ("%' . $recherche . '%") 
							OR b.code_saq like ("%' . $recherche . '%")
							OR LOWER(t.type) like LOWER("%' . $recherche . '%")';
    ...
    /**
     * Cette méthode ajoute une bouteille au cellier depuis la page du catalogue
     * 
     ...
     * */
    public function ajouterDepuisLeCatalogue($data)
    {
        $reponse = ['data' => null];

        // Vérifier si la bouteille à ajouter n'existe pas déjà dans le cellier 
        $sql = "SELECT vino__cellier_id, vino__bouteille_id FROM " . self::CELLIER_BOUTEILLE . " WHERE vino__bouteille_id =" . $data->id_bouteille . " AND vino__cellier_id = " .  $data->id_cellier;
        ...
        if ($this->_db->query($sql)->num_rows > 0) {
            $reponse['data'] = $this->modifierQuantiteBouteilleCellier($data->id_bouteille, $data->id_cellier, 1);
        } else {
            $requete = "INSERT INTO " . self::CELLIER_BOUTEILLE . " (vino__cellier_id,vino__bouteille_id,date_achat,prix,quantite,note_degustation) VALUES (?,?,?,?,1,0)";
            $stmt = $this->_db->prepare($requete);
            $stmt->bind_param('iisd', $data->id_cellier, $data->id_bouteille, $data->date_achat, $data->prix);
    ...
    public function modifierInfoBouteilleCellier($data)
    {
        ...
        if ($data->note_degustation !== "" && $data->note_degustation > 5 || $data->note_degustation < 0) {
            $this->erreurs['note_degustation'] = "Veuillez entrer une note entre 0 et 5.";
        }
        ...
        note_degustation = '$data->note_degustation'
    ...
    public function ajouterBouteilleCellier($data)
    {
        ...
        if (empty($this->erreurs)) {
            ...
            if ($this->_db->query($sql)->num_rows > 0) {
                ...
                $requete = "INSERT INTO " . self::CELLIER_BOUTEILLE . " (vino__cellier_id,vino__bouteille_id,date_achat,garde_jusqua,notes,prix,quantite,millesime,note_degustation) VALUES (?,?,?,?,?,?,?,?,0)";
                $stmt = $this->_db->prepare($requete);
